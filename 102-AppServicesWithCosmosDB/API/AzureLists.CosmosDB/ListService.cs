...
namespace AzureLists.CosmosDB
{
    public class ListService
    {
        ...
        {
            await this.listRepository.DeleteItemAsync(listId);
        }
        public async Task<IEnumerable<List>> GetAllLists()
        {
            var lists = await this.listRepository.GetItemsAsync(x => true);
            return lists;
        }
        public async Task<List> GetListById(string listId)
        {
            var list = await this.listRepository.GetItemAsync(listId);
            return list;
        }
